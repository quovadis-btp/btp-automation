...
// only use the trial postresql plan, the reason being even the free postfresql plan incurs a charge
#
...
data "http" "add_instanceMappings" {
  ...
  count          = local.admin_api_access == {} ? 0 : 1
  depends_on     = [btp_subaccount_service_instance.admin_api_access, data.http.token-cert-admin_api_access, terraform_data.provider_context]
